...
import { STORE_USER, STORE_INVENTORY
} from 'app/constants';
...
export class TwitchApp extends React.Component<TwitchAppProps, TwitchAppState> {
  ...
  getCursorStyle(){
    if(this.props.inventory.itemSelected){
      return {cursor: "url(" + "assets/icons/"+this.props.inventory.selectedItem.icon + ".svg) 2 2, pointer"}
    }else{
      null;
    }
  }
  ...
  render() {
    ...
    if(userStore.isLoaded){
      return (
        <a style= {this.getCursorStyle()} ref={(element) => {this.element = element }} onClick={(e)=>this.clickEvent(e)} className={style.container}>
